...
public interface FeedbackRepository extends JpaRepository<Feedback, Integer> {
	...
	@Query("select new com.aroha.demo.payload.FeedbackPayload(f.feedbackinfo,f.feedbackGivenBy,f.createdOn)from Feedback f left join f.group g where g.groupId=?1")
...
//	public List<FeedbackPayload>showFeedbackforAdmin(int appId);
	...
	@Query(value="select * from feedback_details where app_id=?1",nativeQuery=true)
